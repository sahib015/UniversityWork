 ...
 */
public class HRAllocateReviewersGUI {
    ...
    HRAllocateReviewersGUI(AuthenticationController authenticator, int reviewID) {
    ...
    private void populateMainPanel() {
        ArrayList<Reviewer> values = reviewController.getReviewers();
        ...
        if (values == null) {
            ...
            if (values.isEmpty()) {
                setConstraints(0, 0, 2, 2);
                    mainPanel.add(new JLabel("ERROR: No records found."), constraints);
            }
            String employeeDetails;
            for (Reviewer x : values) {
